 ...
 * @author Mike Gouline
 */
public class TimerViewTrivia extends View {
    ...
    public TimerViewTrivia(Context context) {
        this(context, null);
    }

    public TimerViewTrivia(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TimerViewTrivia(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        int circleColor = Color.RED;

        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.TimerViewTrivia);
            if (ta != null) {
                circleColor = ta.getColor(R.styleable.TimerViewTrivia_circleColor, circleColor);
                ta.recycle();
            }
        }
